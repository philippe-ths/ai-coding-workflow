...
This is the first worked example produced under the periodic review process defined in `design/decisions/evaluation.md`.
...
This proposal is **not for issue creation**. It demonstrates the proposal format using qualitative evidence from `field-notes/observed-ai-failings.md`.

### Proposal 1 (illustrative): scope drift during conflict resolution

- **Classification:** `skill`
- **Problem observed:**
  Multiple `field-notes/observed-ai-failings.md` entries describe the agent silently expanding scope while resolving cherry-pick or merge conflicts — folding adjacent fixes into the resolution commit where review attention is lower than for a normal feature commit.
- **Evidence:**
