...
- `aiw-orchestration` skill in both `.agents/skills/` and `.claude/skills/`: the execution-shape decision for a task, loaded conditionally. It names four shapes — solo in the main loop, read-only scouts, an orchestrator over builders and reviewers, and a relay — and gates work that writes behind a test the workflow previously had no way to fail. Fan out only when the pieces are genuinely independent, meaning no piece needs another's output to start, and at least one of three conditions holds: the inputs exceed one context window, the pieces are mechanical and fully specified so a cheaper capability tier clears the bar, or the edits would collide in one working tree ([#260]).
- The negative test is absolute rather than size-qualified: a dependent chain does not become splittable by being large. Size is what the relay shape answers — the chain is handed along one agent at a time, each briefed from the last one's result, with the main loop holding the thread — so no single window has to hold the whole chain and the dependent pieces are still never split ([#260]).
- Two things sit outside the gate, because both are mandated elsewhere and a gate appearing to govern them would read as weakening an existing rule. Reconnaissance: scouts write nothing, so they open no coordination surface and carry no merge cost, and `aiw-planning` requires them whenever the baseline spans several areas. Review: a reviewer needs the work's output to start and so can never satisfy the independence test, and `aiw-verification` decides when a clean-context or refuting agent is required ([#260]).
- The skill is the only place the conditions live in the full profile. `ai-workflow.md` carries a loader trigger and says outright that it does not restate them; `CLAUDE.md` is a tool mapping that adds no rules; `aiw-planning` requires the plan to name its shape without restating how to choose one. The lite profile ships no skills, so its Execution Shape section is a necessary second copy. One decision rule spread across several hand-maintained files is several copies of the same defect, and this repository's prose check is structural and cannot tell whether two passages contradict each other ([#260]).
- The trigger is the shape of the work, not a decision about it: the skill loads when a task spans many files or areas, exceeds one context window, looks like a batch of mechanical edits, would collide in one tree, or prompts any impulse to hand implementation out, and before handing writing work to any agent other than the main loop. A trigger phrased as "before choosing any shape beyond solo" would consult the gate only after the decision it exists to refuse, and would never fire on a single sequential builder ([#260]).
- The skill routes capability and effort per role, on the distinction that model choice sets the capability ceiling and effort sets thoroughness. A returned result that read the right evidence and drew the wrong conclusion is a capability miss; one that skipped files, tests, or steps is a thoroughness miss. The orchestrator is not routed at all: it is the main loop on whatever model the human chose. Where a tool has no per-spawn effort control, the thoroughness goes into the brief instead. Routing down in capability is permitted only where a sensor exists that can reject bad work, named in the brief ([#260]).
- The skill states what a brief must carry — goal as outcome, oracle and its source, files in and out of scope, the acceptance signal the sub-agent runs itself, and what to report — because a sub-agent's prompt is all the context it has. Reviewers get the requirement, scope, diff, and evidence, never the implementing transcript, which is the withholding `aiw-verification` already requires of its refuting pass, applied to a piece mid-flight ([#260]).
- Concurrency is capped at what the human can still review rather than at what the system can run, since four builders produce four review queues arriving at one person. The cap is stated as a limit on their attention, not as a claim about throughput ([#260]).
- The grounding is the repository's own `design/research/subagents.md`. Anthropic's orchestrator-worker beat a single agent by 90.2% on breadth-first research where subtasks were independent, at roughly 15x the tokens of chat, and explicitly did not claim the gain where subtasks depend on shared intermediate state. Cognition's decomposition argument supplies the failure the gate tests for: pieces split across a decision none of them has made come back with incompatible answers the parent cannot reconcile. Cemri et al. supply the standing cost, that every split opens a coordination surface. The rules therefore say the task must be worth the multiplier, rather than asserting that orchestration is generally more expensive, which the same research contradicts ([#260]).
- `lite-monolithic/ai-workflow.md` receives an Execution Shape section condensing the same gate, shapes, routing, brief, and merge rules, plus the planning requirement. It names no machinery the lite profile does not ship ([#260]).

### Changed

- `ai-workflow.md` Resource Discipline is reduced to two things: reconnaissance routes out freely because it writes nothing, and everything that writes is `aiw-orchestration`'s call. It previously said only when to reach for a sub-agent, so an agent reading it had a rule that fired and no rule that held; it now holds no conditions of its own, which is what keeps it from drifting against the skill ([#260]).
- `CLAUDE.md`'s routing section is a mapping again. It previously offered "multi-step research or delegated implementation: the general-purpose agent" with no qualification, in the file loaded before the workflow, so an unqualified licence to delegate implementation reached the agent first. Research and reconnaissance stay ungated, handing implementation out points at the skill, and the per-role block names Claude Code tiers, worktree isolation, the absence of a per-spawn effort control, and the repository's validation command as the sensor — and states no rules of its own ([#260]).
- `aiw-planning` requires the plan to state its execution shape and why, with solo as the default that needs no argument, and now says that scouting is the one fan-out that pays on almost any task while not being a licence to fan out the implementation. The two sat one section apart with nothing distinguishing them, and read-only reconnaissance and parallel implementation have opposite cost profiles: scouts carry no merge cost, builders carry all of it ([#260]).
- `project-context.md` gains an Execution shape domain concept, names `aiw-orchestration` in Scope and Project Structure, and bumps its `Version:` header to `1.31.0` ([#260]).
...
- `north-star.md`, the shortest statement of what a project is trying to achieve, owned by the new `aiw-north-star` skill. The workflow knew what the system does, from the codebase, and what it should do, from the spec. Nothing said what the project was trying to achieve, so every judgement turning on the project's direction was resolved by interrupting the human. The file holds the goal and nothing else, because it is read at the start of every task and anything in it that does not change a decision is taken from the work ([#244]).
- `aiw-north-star` authors the file by interview: read what exists, bring a candidate goal, ask for the outcome rather than a summary, confirm in plain terms, then write. Writing from inference alone is ruled out, because the goal is why the codebase looks as it does and reading it back out of the artifact is circular. The agent writes the file and the human is its source of truth, the same division `project-context.md` uses with the codebase, except that the codebase cannot supply this one. The skill's altitude test sits between two failures: a goal that describes what the project currently does can never judge it, and a goal no request could contradict decides nothing ([#244]).
- `ai-workflow.md` gains a first Task Flow step. An arriving request is checked against the north-star before planning, and a request that pulls the project away from its goal stops there, because either the request is wrong or the goal has moved and the human decides which. Placing the check before planning is the point: planning around a conflict resolves it silently in favour of the request, and by the time a plan exists the conflict has been designed away ([#244]).
- A first principle: the north-star is the truth about what the project is trying to achieve. The reconciliation rule now lists it alongside code, spec and runtime, so a request pulling against the project's direction is reconciled rather than resolved in the request's favour by default ([#244]).

### Changed

- The Always Do rule on stopping to ask now states that a question the north-star settles is not unclear, while risk and scope always are. Without the second clause the first would race the Ask First list, which is about irreversibility rather than direction and is unchanged ([#244]).
- `aiw-planning`'s Oracle section asks the plan to record what the north-star settled where the approach turns on what the project is trying to achieve. This extends the existing section rather than adding a plan element, because the north-star is an oracle, not a new kind of planning artifact ([#244]).
- `lite-monolithic/ai-workflow.md` receives the principle, both rule changes, the step-1 check, and a condensed North Star section, since it has no skill layer to carry the authoring rules ([#244]).
...
- Added `.ai-policy/hooks/remind-context-management.sh`, an advisory `PreToolUse` hook that points the agent at `aiw-project-context-management` at the two moments `project-context.md` actually goes wrong: before a pull request opens on a branch that changed what the file records without touching it, and before the file is written. The existing signals miss both. `check-context-drift.sh` fires at session start on a commit count, which is too early to act on and easy to forget across a long session, and the task-start instruction to read the file and flag staleness is a soft rule with nothing behind it. It emits `hookSpecificOutput.additionalContext` with **no `permissionDecision` key**, which delivers the reminder and leaves the tool call to the normal permission flow. Emitting `"allow"` would deliver the same text and also auto-approve the call, and on the broad matchers this is registered under that would silently bypass the human's permission prompts for every tool call it fires on, so the key is omitted deliberately rather than by oversight, and the test asserts its absence on every firing. Wired for Claude Code, Codex and VS Code Copilot; **not** Gemini CLI, whose `BeforeTool` consumer does not read `additionalContext` and treats stray stdout as a parse failure, which would trade a reminder that cannot land for a per-call error. That is the same two-of-four compromise `check-context-drift.sh` already carries. Covered by `.ai-policy/scripts/test-context-reminder-hook.sh`, auto-discovered by `project-validation.sh`, which also asserts the wiring in the three configs and asserts Gemini's absence, since a behaviour test passes just as happily when nothing invokes the hook ([#239]).

### Changed

- The pull-request arm fires on the four shapes existing validation genuinely misses, and deliberately not on the two it already catches. A new top-level tracked file fails `scripts/check-manifest.sh`, whose coverage check rejects any tracked file no manifest category claims, and an added skill directory fails `scripts/check-prose-integrity.sh`, which asserts `project-context.md` names every skill in the tree. Putting a second guard behind a gate that holds spends the credibility the real firings need. What nothing sees is the other direction of that same prose check: it walks the skills that exist and confirms the context file names them, never that the skills the context file names still exist, so a skill deleted from both mirrored trees leaves the file naming something gone and validation stays green. Nor does anything see a module added, removed or renamed inside a directory the manifest classifies by prefix, a change to what the policy layer enforces, or a change to what validation covers. The added-skill exclusion was not reasoned into the hook; the first implementation fired on it, and the test written to assert the exclusion is what caught it. The arm is additionally conditioned on the branch not touching `project-context.md`, because a branch that touched it considered the question whatever it concluded ([#239]).
- The message states the signal and never a verdict. The hook can see that the branch touched something the file is supposed to describe; it cannot see whether the file is now wrong, and a meaningful share of firings will be changes that altered nothing it records. So the text names the paths that fired, asks whether they altered what the file records, and ends "If they did not, proceed" — a guard that fires on the normal path gets routed around, and this one is designed to be dismissed without friction when dismissal is right ([#239]).
- `aiw-github` gains one line in its pre-pull-request checklist: whether the branch changed what `project-context.md` records, and if so to refresh it with `aiw-project-context-management`. The line names the class rather than the four shapes the hook matches, so the prose does not become a second copy of the predicate that drifts from it. The rule now exists independently of the hook, which reinforces it rather than being its only carrier. Applied identically to `.claude/skills/` and `.agents/skills/`. `lite-monolithic/ai-workflow.md` has no context document and no policy layer, so its version is bumped to stay in canonical parity with no content change ([#239]).

## 3.27.0 - 2026-08-26

### Changed

- `aiw-project-context-management` now counts its budget in a unit that measures what actually grows, and fires on the edit that actually happens. In a repository running this workflow, `project-context.md` reached 187,923 characters while passing the 300-line cap the whole way. It had been trimmed once already, from 204k to 172k, and seven days and five feature commits later it was back at 188k: **+15,969 characters against +7 lines**, characters per line 834 to 882, median line 301 and longest 11,957. Every feature commit touching the file in that window is `1 add / 1 del` or `5 add / 5 del`, the signature of appending a clause to a line that already exists. The line cap was not a rule that failed to prevent this. It was the rule that shaped it: `Keep the file under 300 lines` and `Write exactly one sentence per line` sat as peers, fusing satisfies the letter of both, and only the line count is checkable at a glance. So the fix is not a character cap racing the other two. One sentence per line now carries the clause that closes the loophole, that a new fact gets its own line rather than extending one that exists, and the cap is restated as a full file where adding means choosing what leaves. The description gains the ship-time trigger, because every phrase in it described a refresh the human asks for while the edit that actually happens is an agent recording work it just shipped, and `## Updating an Existing Context File` opened with "Rerun the scan above", which an agent adding one fact will not do, so the section governing the common case read as inapplicable. That section now leads with the one-fact append and demonstrates the case that produces the damage, where the new fact belongs beside an existing line and extending it is both tempting and invisible. `## Before Handing Off` was three restatements of the base rules and is now the single check that the base rules cannot make, to read the diff and notice a line that got longer where a line should have been added. A fourth rule says how to judge the trade the third one forces, and it was earned rather than assumed: running the same ship-time task against the old and new text, the old text fused the fact into an existing line as a parenthetical and removed nothing, reproducing the production failure exactly, while the new text added a line and then removed a load-bearing domain concept to make room, choosing it as the fact least related to the change in hand. Forcing a trade without saying how to judge it is a worse failure than the crowding it relieves, so the rule now says to drop the line a reader is least likely to act on rather than the line least related to the change being made. Re-running the probe against that wording moved the reasoning from relatedness to what a reader could recover from the code anyway. The second-order cost is what makes this worth more than tidiness: when that file was finally trimmed, six claims it had been asserting turned out to be false, including a documented cache key that was really a partial unique index, so a query written from that line returned duplicate rows. A file too large to verify stops being verified ([#237]).

## 3.26.1 - 2026-08-23

### Fixed

- The guard that rejects a box-ticked unverified surface now catches the form this repository actually writes. It matched only when the trigger phrase and the bare word sat on one line, so `Not verified: none` was blocked while the same declaration under a heading was not. Of the twenty-five most recent merged pull request bodies, twelve declare an unverified surface and **every one of them** uses a heading or a bold lead-in, so the guard was catching the shape nobody writes and missing the shape everybody does. Rather than add a second pattern to keep in step with the first, each line is now also read joined to the next non-empty line and the existing matcher runs over that view as well. Joining cannot invent a match, because the pattern anchors the bare word to the end of the line: a section opening "None of the sync paths were exercised" still has content after the word and does not match. The matcher also tolerates punctuation between the trigger and the bare word, since one real heading in the corpus reads `3. What was not checked.`. A fenced code block is now excluded from this check as well: a body that shows the bad form in order to discuss it is the same case as one that quotes it inline, which the guard already allowed, and the pull request fixing the guard is necessarily written that way. It blocked itself until the fences were excluded. All thirty of the most recent merged bodies were run through the changed guard and none is blocked, which is the same corpus check that established the guard's normal path when it was built ([#235]).
