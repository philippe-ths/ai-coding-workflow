...
- `north-star.md`, a per-repository statement of the goal a project is trying to achieve and the restrictions it must stay inside, owned by the new `aiw-north-star` skill. The workflow knew what the system does, from the codebase, and what it should do, from the spec. Nothing said what the project is for or what it must never become, so every judgement turning on the project's direction was resolved by interrupting the human. The file is agent-written and human-readable, the same division `project-context.md` uses with the codebase, except that its source of truth is the human rather than the code ([#244]).
- `aiw-north-star` authors the file by interview: read what exists, bring a candidate goal and candidate restrictions, ask one question at a time, confirm each in plain terms, then write. Writing it from inference alone is ruled out, because the goal is why the codebase looks as it does and reading intent back out of the artifact is circular. The skill carries the altitude test that keeps restrictions above the constraints they produce: a restriction naming a file, a number, a command or a tool has fallen to the altitude of `project-context.md`'s Important Constraints and should explain why that constraint exists instead of repeating it ([#244]).
- `ai-workflow.md` gains a first Task Flow step. An arriving request is checked against the north-star before planning, and a request that would break the goal or a restriction stops there, because either the request is wrong or the goal has moved and the human decides which. Placing the check before planning is the point: planning around a conflict resolves it silently in favour of the request, and by the time a plan exists the conflict has been designed away ([#244]).
- A first principle: the north-star is the truth about what the project is for and what it must never become. The reconciliation rule now lists it alongside code, spec and runtime, so a request that conflicts with the project's direction is reconciled rather than resolved in the request's favour by default ([#244]).
...
- `aiw-planning`'s Oracle section asks the plan to record what the north-star settled where the approach turns on what the project is for or must not become. This extends the existing section rather than adding a plan element, because the north-star is an oracle, not a new kind of planning artifact ([#244]).
