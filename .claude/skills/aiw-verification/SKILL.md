...
description: "Governs how the agent confirms a change actually works before declaring a task complete. Use this skill at the end of every implementation pass and every time the agent is about to say 'done', 'this works', 'tests pass', 'ready to commit', or 'ready to push'. Also use when interpreting runtime output, when deciding whether to run the full system end-to-end, when claiming a refactor preserves behaviour, when claiming a fix is fixed, when claiming a migration is complete, when claiming a delete is safe, and when assessing whether the checks the agent has run are sufficient evidence for the change. The skill exists to break the 'all tests pass, system is broken' failure mode by forcing the agent to justify, every time, that its checks cover the change and to name the unverified surface rather than hide it. It owns the required justification step before any task can be declared complete, the evidence hierarchy from static checks through end-to-end runs on real artifacts, the rules for when end-to-end execution is mandatory, how to read runtime output as evidence (not exit codes), modality-aware verification requirements that reference the modality procedure in aiw-ground-truth, and the scoping step that names what was not checked and the risk that implies."
---
