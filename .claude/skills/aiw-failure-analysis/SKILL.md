...
description: "Reactive skill for the moment when the agent has claimed a task complete and that claim has been contradicted by the user, by runtime behaviour, or by manual verification. Use this skill whenever the user reports the behaviour is still broken, the fix didn't help, the system is doing the wrong thing, or runtime behaviour disagrees with what the agent's tests or validation reported. Common surface phrases: 'still broken', 'still doesn't work', 'still fails', 'didn't help', 'didn't work', 'the bug remains', 'not working', 'doing the wrong thing'. The underlying trigger is a trust collapse — the three core skills (aiw-ground-truth, aiw-testing, aiw-verification) that should have caught the problem did not, and the agent can no longer trust its oracle, its tests, or its verification conclusion until each has been audited. Load this skill BEFORE proposing any fix to the contradiction. The skill exists to prevent the agent from jumping to the nearest plausible patch when the plausible patch is likely also wrong: if understanding was wrong enough for verification to miss the problem, the next attempt under the same understanding will miss it again. The skill owns the structured pause after a contradicted 'done' claim, the audit of the three core skills to locate where the gap opened, the discipline of describing the contradiction before reasoning about it, the hypothesis-and-evidence loop that replaces speculative fixing, the convergence check for when repeated fixes are not working, and the plan-level flaw detection path when the original plan was based on wrong assumptions about the codebase or system."
---
