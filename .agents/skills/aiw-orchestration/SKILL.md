...
**Step owner.** When the built system has a step a model runs on every use, one that judges, drafts, classifies or proposes, that step is a standing sub-agent with its own design: one prompt file that names its role, sealed from the always-on layer; a narrow, named interface in and out, files where the system is a pipeline; and a log of every run recording what the step cost and the verdict on what it produced, from the human or from a check, so its context is tuned against numbers and a change to its prompt can be rerun on the same inputs and compared. The step recurs, so its context is a design decision made once at planning rather than what has accumulated in the files around it by the time it runs. The always-on files describe the pipeline; the step's own prompt performs it. The gate above does not apply: this is not the task splitting, it is the task deciding who runs a step of the thing it is building. aiw-planning names the owner in the plan. Its prompt is written with aiw-prompt-smith and carries what The Brief below asks of any sub-agent, stated once for every run rather than per task.
