...
The four shapes above run a task. The fifth belongs to the system a task builds, and the solo default above is about the task's own judgement, which stays in the main loop; a step's judgement belongs to the system and runs without you.

**Step owner.** When the built system has a step a model runs on every use, one that judges, drafts, classifies or proposes, that step is a standing sub-agent and is designed like one: a prompt file of its own that names its role, sealed from the always-on layer; a narrow, named interface in and out; and a log of every run recording what the step cost and the verdict on what it produced, from the human or from a check, so that its context is tuned against numbers and a change to its prompt can be rerun on the same inputs and compared. Its context is decided once, at planning, not accumulated. The always-on files describe the pipeline; the step's own prompt performs it. *The hard case has no files.* A model that labels each support ticket inside a request handler still has an owner: its interface is the three request fields it reads and the one label it returns, its prompt is the file the handler loads and nothing else, and its log is a row per call. The gate above does not apply: this is not the task splitting, it is the task deciding who runs a step of the thing it is building. aiw-planning names the owner in the plan; the prompt is written with aiw-prompt-smith and carries what The Brief below asks of any sub-agent, stated once for every run.
...
| Step owner | What the step's judgement needs; a cheaper tier only where a sensor rejects its bad output | Set once, then read from the run log | The step runs on every use, so tier and effort compound, and the log is what shows whether either was wrong |
