---
name: aiw-planning
description: "Structured planning process for producing a code-aware implementation plan before writing code, including the pre-planning codebase baseline checks (smoke tests and the global suite, test readiness, bounded-change confirmation) and the task modality classification on which the rest of the plan depends. Use this skill at the start of every task before any implementation begins, whether the user says 'plan' or just hands the agent an issue to work on. The skill exists to prevent the agent from jumping straight into coding without establishing what currently works in the codebase, what kind of task is actually being asked (using the modality procedure in aiw-ground-truth), what will count as correct for the task (the oracle, again referencing aiw-ground-truth), and how the change will be verified during implementation (referencing aiw-verification). The skill is the entry point that hooks the rest of the workflow into the task: it owns the codebase baseline check, the modality classification step, the plan's structure and required content, the issue-vs-codebase priority rule, assumption classification, bounded-change confirmation, and the rules for handling human feedback at plan review. It does not own oracle definitions or the trust hierarchy (that's aiw-ground-truth), test mechanics (that's aiw-testing), the evidence hierarchy or verification sufficiency (that's aiw-verification), or post-failure investigation (that's aiw-failure-analysis)."
---
