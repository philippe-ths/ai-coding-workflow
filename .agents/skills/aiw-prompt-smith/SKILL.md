...
4. **Layer**: who reads this line, in which file, with what else in their context at the time? Altitude is about the line; layer is about where it lives, and it is a decision, not a default. What nearly every session needs goes always-on; a method pulled on match goes in a skill; what is looked up goes in a reference; the role and judgement of one step a model runs go in that step's own prompt, sealed from the rest (aiw-orchestration's step owner). A correct line at the wrong layer is either not read or read alongside so much else that it does not bind, and the common way there is prose written for a human reader and handed to a model unchanged.
