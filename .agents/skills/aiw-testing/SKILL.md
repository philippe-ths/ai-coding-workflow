---
name: aiw-testing
description: "Mechanical craft rules for writing tests well: choosing the right test level, consuming ground truth from aiw-ground-truth rather than fabricating it, organising tests for independence and clarity, and avoiding the anti-patterns that produce green test suites with no real authority. Use this skill whenever the agent is about to write a new test, modify an existing test, choose a fixture, pick a test level (unit, integration, end-to-end), or decide whether to mock a component. Also use when refactoring test code, when filling a coverage gap identified by aiw-verification, when reorganising test files, when writing a regression test for a bug, and when the agent is tempted to assert on internal method calls or private state. The skill exists for the narrow problem of writing tests well; it does not decide what counts as correct (that belongs to aiw-ground-truth) or whether the tests are sufficient evidence (that belongs to aiw-verification). It owns test mechanics, test-level selection, modality-specific testing emphasis that references the modality procedure in aiw-ground-truth, the anti-patterns that produce misleading green bars, and the hygiene rules that keep a test suite a reliable signal rather than a self-confirming loop."
---
