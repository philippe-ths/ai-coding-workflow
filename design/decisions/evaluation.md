...
5. **`.envrc` `OTEL_RESOURCE_ATTRIBUTES` matches the values recomputed from current files.** `workflow_version` must equal `ai-workflow.md`'s `Version:` header; `ruleset_hash` must equal a fresh recomputation across the ruleset files (`ai-workflow.md`, `CLAUDE.md`, `.ai-policy/policy.env`, `.ai-policy/hooks/*.sh`, `.claude/skills/*/SKILL.md`). Catches the recurring staleness class where a workflow bump or rule edit was never propagated to `.envrc`, so the current session's data is being emitted under the wrong version tag.
6. **The baseline harness can resolve `workflow_version` and `ruleset_hash` at import time.** `evals/harness/context.py` must execute both functions without raising. Catches code-side breakage in the harness (wrong source file, missing dependency, schema drift) before the review walks into it.
