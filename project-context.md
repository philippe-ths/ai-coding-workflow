...
- Vendors two third-party skills (`improve-codebase-architecture` and `grill-with-docs`, from `mattpocock/skills`) in both skill directories; they surface codebase architecture and refactoring opportunities and stress-test plans against domain language, and are not part of the `aiw-*` workflow set.
...
- `improve-codebase-architecture/` (in both skill directories): vendored third-party skill from `mattpocock/skills`; a multi-file skill (`SKILL.md` plus `DEEPENING.md`, `HTML-REPORT.md`, `INTERFACE-DESIGN.md`, `LANGUAGE.md`) for finding architecture deepening opportunities, not part of the `aiw-*` set; references the sibling `grill-with-docs` skill.
- `grill-with-docs/` (in both skill directories): vendored third-party skill from `mattpocock/skills`; a multi-file skill (`SKILL.md` plus `CONTEXT-FORMAT.md`, `ADR-FORMAT.md`) that grills a plan against the domain model and updates `CONTEXT.md`/ADRs inline, not part of the `aiw-*` set.
